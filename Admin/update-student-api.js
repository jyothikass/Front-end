function apiCreateNewInvoice(invoice, form) {
    ...
    axios.post('http://localhost:8280/invoice/', invoice, { headers })
            .then(res => {
                    form.reset()
                    showSuccessModal()
            })
...
function apiCreateNewCustomerInvoive(invoice, form) {
    ...
    axios.post('http://localhost:8280/invoice/customer', invoice, { headers })
            .then(res => {
                    form.reset()
                    showSuccessModal()
            })
...
function apiFindCustomersContaining(ss) {
    const url = `http://localhost:8280/customer/${ss}`
    axios.get(url)
